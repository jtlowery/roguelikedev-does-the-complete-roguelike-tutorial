...
class Rect:
    ...
    def intersect(self, other):
        ...
        within_x = self.x1 <= other.x2 and self.x2 >= other.x1
        within_y = self.y1 <= other.y2 and self.y2 >= other.y1
...
def place_entities(room, entities, max_monsters_per_room, max_items_per_rooms, colors):
    # get a random number of monsters
    number_of_monsters = randint(0, max_monsters_per_room)
    number_of_items = randint(0, max_items_per_rooms)
...
def make_map(game_map,
    ...
    for r in range(max_rooms):
        ...
        for other_room in rooms:
            ...
            create_room(game_map=game_map, room=new_room)
            ...
            if num_rooms == 0:
                ...
                if randint(0, 1) == 1:
                    ...
                    create_h_tunnel(game_map=game_map,
                                    x1=prev_x,
                                    x2=new_x,
                                    y=prev_y)
                    create_v_tunnel(game_map=game_map,
                                    y1=prev_y,
                                    y2=new_y,
                                    x=new_x)
                else:
